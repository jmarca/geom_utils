...
var get_area = require('./get_area').get_area
...
function get_bbox(req,area_type,area_param){
    var bbox;
    var bb;
    if(req.params.bbox){
        bbox = req.param('bbox')
    ...
    if( /(-?\d+\.\d+).*?(-?\d+\.\d+).*?(-?\d+\.\d+).*?(-?\d+\.\d+)/.exec(bb)){
        ...
        bbox = get_area(req,area_type,area_param)
    }
    return bbox;
}
function get_bbox_with_format(req,alias,area_type,area_param){
    if(alias===undefined){alias='bounding_area'}
    var bbox =  get_bbox(req,area_type,area_param)
    if(bbox === undefined)
        return bbox
